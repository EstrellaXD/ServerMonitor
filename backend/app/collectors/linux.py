...
class LinuxCollector(BaseCollector):
    ...
    def _parse_combined_output(self, output: str) -> dict[str, str]:
        """Parse combined command output into sections."""
        sections = {}
    ...
    async def collect(self) -> SystemMetrics:
        ...
        # Combined command to fetch all metrics in a single SSH round-trip
        combined_cmd = (
            ...
            '[ -n "$temp" ] && echo "$name:$temp"; done 2>/dev/null || true)'
        )

        conn = await self._get_connection()
        combined_output = await self._run_command(conn, combined_cmd)

        # Parse combined output into sections
        sections = self._parse_combined_output(combined_output)

        stat = sections.get("STAT", "")
        meminfo = sections.get("MEMINFO", "")
        df = sections.get("DF", "")
        netdev = sections.get("NETDEV", "")
        uptime = sections.get("UPTIME", "")
        loadavg = sections.get("LOADAVG", "")
        sensors = sections.get("SENSORS", "")
        thermal_zones = sections.get("THERMAL", "")
