...
const secondaryMetric = computed(() => {
  ...
  switch (props.system.type) {
    case 'linux': {
      const m = metrics as LinuxMetrics
      return { label: 'RAM', value: `${m.memory?.percent?.toFixed(1) ?? 0}%` }
    }
    case 'docker': {
      const m = metrics as DockerMetrics
...
<template>
  ...
  >
    ...
    <!-- Hover indicator -->
    <div class="mt-4 flex items-center gap-1 text-sm text-slate-400 dark:text-slate-500 opacity-0 group-hover:opacity-100 transition-opacity">
